        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="flex items-start justify-between p-6 border-b border-white/10">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h2 className="text-lg font-bold text-gray-100">{complaint.title || complaint.category}</h2>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="font-mono text-xs text-indigo-600">{complaint.complaintId}</span>
                  <StatusBadge status={complaint.isActive === false ? 'Deleted by Admin' : complaint.status} />
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1 text-xs px-2.5 py-1.5 border border-white/15 rounded-lg text-gray-300 hover:bg-white/5"
                  title="Download PDF"
                >
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-6 space-y-5">
              ...
              {(complaint.isActive === false || complaint.rejectionReason) && (
                <div className="flex items-start gap-2 p-3 rounded-xl bg-red-500/10 border border-red-500/25 text-sm">
                  <AlertTriangle size={15} className="text-red-400 flex-shrink-0 mt-0.5" />
                  <div>
                    <span className="font-semibold text-red-400">
                      {complaint.isActive === false ? 'Removed by Admin' : 'Complaint Rejected'}
                    </span>
                    {complaint.rejectionReason && (
                      <p className="text-red-300/80 text-xs mt-0.5">{complaint.rejectionReason}</p>
                    )}
                  </div>
                </div>
              )}
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {complaint.images.map((img, i) => (
                      <img
                        key={i}
                        src={img}
                        className="w-full aspect-square object-cover rounded-lg cursor-pointer hover:opacity-90 border border-white/10"
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {complaint.videos.map((vid, i) => (
                      <video key={i} src={vid} controls className="w-full rounded-xl border border-white/10 max-h-48" />
                    ))}
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {complaint.assignedTo && (
                  <div className="p-3 bg-indigo-500/10 rounded-xl col-span-2">
                    <div className="text-xs text-indigo-400 mb-0.5">Assigned Worker</div>
                    <div className="text-sm font-medium text-indigo-300 flex items-center gap-1">
                      <User size={12} />
                  ...
                  {complaint.aiAnalysis.detectedObjects?.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {complaint.aiAnalysis.detectedObjects.slice(0, 5).map((obj, i) => (
                        <span key={i} className="text-[11px] bg-white/5 border border-violet-500/30 rounded-full px-2 py-0.5 text-violet-300">
                          {obj.name}
                        </span>
                      ))}
                    </div>
              ...
              {complaint.description && (
                <div>
                  ...
                  <p className="text-sm text-gray-400 bg-white/[0.04] p-3 rounded-xl">
                    {complaint.description}
                  </p>
                  {translatedDesc && (
                    <div className="mt-2 bg-indigo-500/10 border border-indigo-500/20 p-3 rounded-xl">
                      <p className="text-xs text-indigo-400 mb-1">Translated to {translatedLang}:</p>
                      <p className="text-sm text-indigo-200">{translatedDesc}</p>
                    </div>
                  )}
                </div>
              )}
